    ...
    def runall(self,cls:Type,feats,targets,**kw):
        ...
        for epoch in range(maxinter):
            ...
            if self.Jloss(out,torch.abs(g.vars)) is not None:
                loss=nn.MSELoss()(out,torch.abs(g.vars))#+(out-torch.abs(g.vars)).mean()
            else:
                loss=self.Jloss(out,torch.abs(g.vars))
            losses.append(loss.item())
            loss.backward()
            if not hasattr(self,'_costmGradown'):
                optimizer.step()
            else:
                self._costmGradown(model=g,lr=50.)
...
def _contini(self, sumup: torch.Tensor, scalar, z0):
    ...
    return A          # (N,1)
def _costmGradown(self,model,lr):
    '''
    ...
    if hasattr(model,'up'):
        ...
        eps=((model.hat-model.vars)/N).sum()
        dLdbias=torch.sgn(eps)*dYdalpha.sum()/N
        dLdalpha=(dYdalpha*dLdy).sum()+dLdbias
        with torch.no_grad():
            model.alphaNL.data.sub_(lr*dLdalpha.real)
def Jloss(self,yhat,y):
    '''
    the factor yhat-y is to make the sum of these values which below fitted curve
    as close as possible to the sum of values which above fitted curve

    this way follows the principle of fitting a curve instead of machine learning
    '''
    loss=nn.MSELoss()(yhat,y)+(yhat-y).mean()
    return loss
